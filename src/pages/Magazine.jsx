...
const Page = forwardRef((props, ref) => {
    return (
        <div className="page-content bg-white text-primary italic overflow-hidden shadow-inner border-r border-gray-200" ref={ref}>
            <div className="h-full w-full relative flex flex-col">
                {props.children}
                {/* Page Number */}
                {props.number && (
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[10px] text-gray-500 font-mono not-italic bg-white/80 border border-gray-200 px-2 rounded-full backdrop-blur-sm z-50">
                        - {props.number} -
                    </div>
                )}
            </div>
...
const CategoryGallery = ({ category, products, onClose, onSelectProduct }) => (
    <div className="fixed inset-0 z-[200] bg-white/95 backdrop-blur-md flex flex-col animate-fadeIn">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-white/50">
            <div>
                <h2 className="text-3xl font-bold text-primary tracking-tight">{category}</h2>
                <p className="text-sm text-gray-500">Ürün Kataloğu</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
                {products.map(product => (
                    <div
                        ...
                        className="group cursor-pointer bg-white rounded-xl border border-gray-200 overflow-hidden hover:border-secondary/50 hover:shadow-lg hover:shadow-secondary/10 transition-all duration-300"
                    >
                        ...
                        <div className="p-4 bg-gray-50 border-t border-gray-100">
                            ...
                            <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider">{product.features.Marka || 'Alya Grup'}</p>
                        </div>
                    </div>
                ))}
            </div>
...
const ProductDetail = ({ product, onBack, onClose }) => (
    <div className="fixed inset-0 z-[210] bg-white flex flex-col md:flex-row animate-slideUp">
        {/* Close/Back Controls */}
        <div className="absolute top-6 right-6 z-50 flex gap-4">
            <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur border border-gray-200 rounded-full text-primary hover:bg-gray-100 transition-all shadow-sm">
                <ArrowLeft size={18} /> <span className="text-sm font-medium">Geri Dön</span>
            </button>
            <button onClick={onClose} className="p-2 bg-white/80 backdrop-blur border border-gray-200 rounded-full text-primary hover:bg-red-50 hover:text-red-500 transition-all shadow-sm">
                <X size={24} />
            </button>
        </div>

        {/* Image Section */}
        <div className="w-full md:w-1/2 h-1/2 md:h-full relative bg-gray-50 flex items-center justify-center">
            <img
                src={product.image}
                alt={product.name}
        ...
        {/* Info Section */}
        <div className="w-full md:w-1/2 h-1/2 md:h-full p-8 md:p-16 flex flex-col justify-center bg-white border-l border-gray-100 overflow-y-auto">
            <span className="text-secondary tracking-widest uppercase text-sm font-bold mb-2">{product.category}</span>
            <h1 className="text-4xl md:text-5xl font-black text-primary mb-6 leading-tight">{product.name}</h1>

            <p className="text-gray-600 text-lg leading-relaxed mb-10 border-l-4 border-gray-200 pl-6 italic">
                "{product.description}"
            </p>

            {/* Features Table */}
            <div className="bg-gray-50 rounded-xl border border-gray-200 p-6 max-w-lg">
                ...
                <div className="space-y-3">
                    {Object.entries(product.features).map(([key, value]) => (
                        <div key={key} className="flex justify-between items-center border-b border-gray-200 pb-2 last:border-0 last:pb-0">
...
const CoverPage = () => (
    <div className="h-full flex flex-col items-center justify-center text-center p-8 bg-white text-primary bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-50 via-white to-gray-100 relative overflow-hidden border-r-4 border-gray-200">
        ...
        <div className="relative z-10 mb-6 sm:mb-16 transform transition-transform duration-700 hover:scale-105">
            <img
                src={AlyaGrupLogo}
                alt="Alya Grup Logo"
                className="w-36 sm:w-72 h-auto object-contain drop-shadow-[0_4px_10px_rgba(0,0,0,0.1)]"
            />
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center gap-2 sm:gap-4">
            <div className="flex items-center gap-4 w-full justify-center">
                <div className="h-[1px] w-8 sm:w-12 bg-gradient-to-r from-transparent to-gray-300"></div>
                <h2 className="text-xs sm:text-lg font-light tracking-[0.3em] text-gray-500 uppercase">
                    Katalog
                </h2>
                <div className="h-[1px] w-8 sm:w-12 bg-gradient-to-l from-transparent to-gray-300"></div>
            </div>

            <h1 className="text-lg sm:text-3xl font-medium tracking-widest text-primary uppercase drop-shadow-sm">
                Temizlik <span className="text-secondary font-bold">&</span> Hijyen
            </h1>

            <p className="text-[9px] font-mono text-gray-400 tracking-[0.2em] mt-1">
        ...
        <div className="absolute bottom-6 sm:bottom-12 left-0 right-0 text-center">
            <div className="inline-block px-4 py-1 border border-gray-200 rounded-full bg-white/50 backdrop-blur-sm shadow-sm">
...
const InfoPage1 = () => (
    <div className="h-full flex flex-col bg-white text-primary">
        <div className="h-1/2 relative overflow-hidden bg-gray-50">
            <img
                src="https://images.unsplash.com/photo-1532187863486-abf9dbad1b69?auto=format&fit=crop&q=80&w=800"
                alt="Lab"
                className="w-full h-full object-cover opacity-90 mix-blend-multiply"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-white via-white/70 to-transparent flex items-end p-6">
                <h2 className="text-3xl font-bold text-primary border-b-2 border-secondary pb-2 bg-white/50 backdrop-blur-sm px-2">Üretim Gücü</h2>
            </div>
        </div>
        <div className="h-1/2 p-8 flex flex-col justify-center border-t border-gray-100">
...
const InfoPage2 = () => (
    <div className="h-full flex flex-col bg-white text-primary">
        ...
        <div className="h-1/2 p-4 sm:p-8 flex flex-col justify-center border-t border-gray-100">
            ...
            <div className="space-y-3 sm:space-y-6">
                <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                        Ankara içi: <span className="text-primary font-bold">24 Saat</span><br />
                        İstanbul: <span className="text-primary font-bold">48 Saat</span><br />
                        Tüm Türkiye: <span className="text-primary font-bold">96 Saat</span>
                    </p>
                </div>
                <div className="text-right">
                    <h4 className="font-bold text-secondary text-sm sm:text-base">Merkezi Dağıtım</h4>
                    <p className="text-xs text-gray-500 mt-1">Ankara merkezli lojistik ağımız ile kesintisiz tedarik zinciri.</p>
                </div>
            </div>
        </div>
    </div>
...
const CategoryCoverPage = ({ category, onOpenGallery }) => (
    <div className="h-full flex flex-col items-center justify-center bg-gray-50 p-8 border-4 border-double border-gray-200 shadow-inner group">
        <div className="w-full h-full border border-gray-200 flex flex-col items-center justify-center bg-white backdrop-blur-sm relative overflow-hidden shadow-sm">

            {/* Background Decoration */}
            <div className="absolute top-0 right-0 p-24 bg-secondary/5 rounded-full blur-2xl translate-x-1/2 -translate-y-1/2"></div>

            <h2 className="text-4xl font-bold text-center text-primary mb-4 px-4 relative z-10">{category}</h2>
            <div className="w-16 h-1 bg-secondary mb-6 relative z-10"></div>
            <p className="text-center text-gray-600 max-w-xs text-sm relative z-10 mb-8">
            ...
            <button
                ...
                className="relative z-10 group/btn flex items-center gap-2 px-6 py-3 bg-primary text-white hover:bg-secondary hover:text-white transition-all duration-300 rounded-full font-bold text-xs uppercase tracking-widest shadow-[0_4px_14px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_20px_rgba(14,165,233,0.4)]"
            >
                <Grid size={16} />
                Tüm Ürünleri İncele
            </button>
...
const ProductGridPage = ({ products, category, pageIndex, totalPages }) => (
    <div className="h-full w-full p-3 sm:p-6 md:p-8 flex flex-col bg-white overflow-hidden box-border">
        {/* Header */}
        <div className="flex justify-between items-end mb-2 sm:mb-4 border-b border-gray-200 pb-2 shrink-0">
            <h3 className="text-sm sm:text-xl font-bold text-primary tracking-tight truncate max-w-[70%]">{category}</h3>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-4 flex-grow content-start">
            {products.map(product => (
                <div key={product.id} className="flex flex-col gap-1 mb-1 group">
                    <div className="aspect-square bg-gray-50 rounded-lg overflow-hidden border border-gray-200 relative shadow-sm hover:border-gray-300 transition-all">
                        <img
                            src={product.image}
                            alt={product.name}
                            className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500 mix-blend-multiply"
                        />
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-white via-white/90 to-transparent p-1 sm:p-2 pt-4 sm:pt-6">
                            <div className="text-primary text-[8px] sm:text-[10px] font-bold truncate text-center drop-shadow-sm">
                                {product.name}
                            </div>
                        </div>
                    </div>
        ...
        {/* Footer */}
        <div className="mt-auto pt-1 sm:pt-2 border-t border-gray-200 flex justify-between items-center text-[7px] sm:text-[8px] text-gray-500 uppercase tracking-widest font-sans not-italic">
            <span>AlyaGrup</span>
            <span>2026</span>
        </div>
    </div>
...
const ContactPage = () => (
    <div className="h-full bg-white text-primary p-8 md:p-12 flex flex-col justify-center relative overflow-hidden">
        ...
        <div className="space-y-8 relative z-10 pl-2">
            <div className="flex items-center gap-4 group">
                <div className="p-3 bg-gray-50 rounded-lg group-hover:bg-secondary/10 transition-colors border border-gray-100"><Phone size={20} className="text-secondary" /></div>
                <div>
            ...
            <div className="flex items-center gap-4 group">
                <div className="p-3 bg-gray-50 rounded-lg group-hover:bg-secondary/10 transition-colors border border-gray-100"><Mail size={20} className="text-secondary" /></div>
                <div>
            ...
            <div className="flex items-center gap-4 group">
                <div className="p-3 bg-gray-50 rounded-lg group-hover:bg-secondary/10 transition-colors border border-gray-100"><MapPin size={20} className="text-secondary" /></div>
                <div>
        ...
        <div className="mt-auto pt-8 border-t border-gray-200 text-[10px] text-gray-500 text-center font-sans not-italic">
            www.alyagrup.com
        </div>
    </div>
);
...
const Magazine = () => {
    ...
    return (
        <div className="h-screen w-full bg-gray-100 flex justify-center items-center overflow-hidden relative">

            {/* Nav Buttons (Edge Click Zones) */}
            <button
                onClick={prevPage}
                className="absolute left-0 top-0 h-full w-[15%] z-[100] outline-none hover:bg-black/5 transition-colors cursor-w-resize"
                title="Önceki"
            />
            <button
                onClick={nextPage}
                className="absolute right-0 top-0 h-full w-[15%] z-[100] outline-none hover:bg-black/5 transition-colors cursor-e-resize"
                title="Sonraki"
            />
            ...
            <div className={`md:hidden absolute bottom-8 left-0 right-0 text-center text-gray-500 text-xs pointer-events-none z-20 ${activeCategory ? 'opacity-0' : ''}`}>
